<?php

namespace App\Exports;

use App\Models\Pesanan;
use Carbon\Carbon;
use Illuminate\Support\Facades\DB;
use Maatwebsite\Excel\Events\AfterSheet;
use Maatwebsite\Excel\Concerns\WithEvents;
use Maatwebsite\Excel\Concerns\WithMapping;
use Maatwebsite\Excel\Concerns\WithHeadings;
use Maatwebsite\Excel\Concerns\FromCollection;
use Maatwebsite\Excel\Concerns\ShouldAutoSize;


class PendapatanExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents
{
    /**
    * @return \Illuminate\Support\Collection
    */
    // public function view(): View
    // {
    //     $pesanan = Pesanan::select(
    //         DB::raw("(DATE_FORMAT(created_at, '%d %M %Y')) as tanggal"),
    //         DB::raw("(count(id)) as alias_total_pesanan"),
    //         DB::raw("(sum(total_pembayaran)) as alias_total_pendapatan_kotor"),
    //         DB::raw("(sum(total_ongkir)) as alias_total_ongkir"),
    //         )
    //         ->orderBy('created_at')
    //         ->groupBy(DB::raw("DATE_FORMAT(created_at, '%d-%m-%Y')"))->get();

    //         return view('Back.laporan.print._print_laporan_pendapatan', compact('pesanan'));
    // }
    public function collection()
    {
        $t_awal = request()->input('tgl_awal') ;
        $t_akhir   = request()->input('tgl_akhir') ;

         $data = Pesanan::select(
            DB::raw("(DATE_FORMAT(created_at, '%d %M %Y')) as tanggal"),
            DB::raw("(count(id)) as alias_total_pesanan"),
            DB::raw("(sum(total_pembayaran)) as alias_total_pendapatan_kotor"),
            DB::raw("(sum(total_ongkir)) as alias_total_ongkir"),
            )
            ->orderBy('created_at')
            ->groupBy(DB::raw("DATE_FORMAT(created_at, '%d-%m-%Y')"));

            if($t_awal && $t_akhir){
                $tgl_awal = Carbon::parse($t_awal);
                $tgl_akhir = Carbon::parse($t_akhir);
                if($tgl_akhir->greaterThan($tgl_awal)){
                    $data = $data->whereBetween('created_at', [$tgl_awal, $tgl_akhir])->get();
                }
            }else{
                $data = $data->get();
            }

        return $data;
    }

    public function map($catatan) : array {
        return [
            [
                Carbon::parse($catatan->tanggal)->format('d F y'),
                $catatan->alias_total_pesanan,
                $this->rupiahFormat($catatan->alias_total_pendapatan_kotor),
                $this->rupiahFormat($catatan->alias_total_ongkir),
                $this->rupiahFormat($catatan->alias_total_pendapatan_kotor - $catatan->alias_total_ongkir),
            ],

        ];
    }

    public function headings() : array {
        return [
            'Tanggal',
            'Pesanan',
            'Pendapatan Kotor',
            'Pengiriman',
            'Pendapatan Bersih'
        ] ;
    }

    public function rupiahFormat($expression)
    {
        return 'Rp.'.number_format($expression,0,',','.');
    }

    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                $t_awal = request()->input('tgl_awal') ;
                $t_akhir   = request()->input('tgl_akhir') ;

                 $data = Pesanan::select(
                    DB::raw("(DATE_FORMAT(created_at, '%d %M %Y')) as tanggal"),
                    DB::raw("(count(id)) as alias_total_pesanan"),
                    DB::raw("(sum(total_pembayaran)) as alias_total_pendapatan_kotor"),
                    DB::raw("(sum(total_ongkir)) as alias_total_ongkir"),
                    )
                    ->orderBy('created_at')
                    ->groupBy(DB::raw("DATE_FORMAT(created_at, '%d-%m-%Y')"));

                    if($t_awal && $t_akhir){
                        $tgl_awal = Carbon::parse($t_awal);
                        $tgl_akhir = Carbon::parse($t_akhir);
                        if($tgl_akhir->greaterThan($tgl_awal)){
                            $data = $data->whereBetween('created_at', [$tgl_awal, $tgl_akhir])->get();
                            $sum_pesanan = $data->sum('alias_total_pesanan');
                            $sum_pendapatan_kotor = $data->sum('alias_total_pendapatan_kotor');
                            $sum_pengiriman = $data->sum('alias_total_ongkir');
                            $total_pendapatan_bersih = $sum_pendapatan_kotor-$sum_pengiriman;
                        }
                    }else{
                        $data = $data->get();
                        $sum_pesanan = $data->sum('alias_total_pesanan');
                        $sum_pendapatan_kotor = $data->sum('alias_total_pendapatan_kotor');
                        $sum_pengiriman = $data->sum('alias_total_ongkir');
                        $total_pendapatan_bersih = $sum_pendapatan_kotor-$sum_pengiriman;
                    }


                $sheet = $event->sheet->getDelegate();
                $sheet->insertNewRowBefore(1,3);
                $lastContentColumn = (string) $event->sheet->getDelegate()->getHighestColumn();
                $lastContentRow = (string) $event->sheet->getDelegate()->getHighestRow();
                $lastContentRowTotal = (string) $event->sheet->getDelegate()->getHighestRow();
                $cellE1 = (string) $lastContentColumn . '1';
                $cellELast = (string) 'E' . $lastContentRow;


                $stringTotal = (string) 'A'. $lastContentRowTotal+1;
                $sumPesanan = (string) 'B'. $lastContentRowTotal+1;
                $sumPendapatanKotor = (string) 'C'. $lastContentRowTotal+1;
                $sumPengiriman = (string) 'D'. $lastContentRowTotal+1;
                $sumPendapatanBersih = (string) 'E'. $lastContentRowTotal+1;

                // JUDUL
                $event->sheet->getDelegate()
                    ->setCellValue('A1','LAPORAN PENDAPATAN')->mergeCells('A1:E1');

                // PERIODE
                $event->sheet->getDelegate()
                    ->setCellValue('A2','Periode : '.Carbon::parse($t_awal)->format('d F y').' - '.Carbon::parse($t_akhir)->format('d F y'))->mergeCells('A2:E2');

                // TOTAL FOOTER
                $event->sheet->getDelegate()
                    ->setCellValue($stringTotal, "Total");

                // SUM PESANAN
                $event->sheet->getDelegate()
                    ->setCellValue($sumPesanan, $sum_pesanan);

                // SUM PENJUALAN FOOTER
                $event->sheet->getDelegate()
                    ->setCellValue($sumPendapatanKotor, $this->rupiahFormat($sum_pendapatan_kotor));

                // SUM PENGIRIMAN FOOTER
                $event->sheet->getDelegate()
                    ->setCellValue($sumPengiriman, $this->rupiahFormat($sum_pengiriman));

                // SUM PENDAPATAN BERSIH FOOTER
                $event->sheet->getDelegate()
                    ->setCellValue($sumPendapatanBersih, $this->rupiahFormat($total_pendapatan_bersih));

                // JUDUL
                $event->sheet->getDelegate()->getStyle("A1:E1")->applyFromArray([
                    'font' => [
                        'bold' => true,
                        'size' => 15,
                    ],
                    'alignment' => [
                        'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                        'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                    ],
                ]);

                 // PERIODE
                 $event->sheet->getDelegate()->getStyle("A2:E2")->applyFromArray([
                    'font' => [
                        'bold' => true,
                        'size' => 12,
                    ],
                    'alignment' => [
                        'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                        'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                    ],
                ]);


                // HEADER
                $event->sheet->getDelegate()->getStyle("A4:E4")->applyFromArray([
                    'font' => [
                        'bold' => true,
                        'size' => 12,
                    ],
                    'borders' => [
                        'right'=>[
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                        ]
                    ],
                    'alignment' => [
                        'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                        'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                    ],
                ]);

                // ISI
                $event->sheet->getDelegate()->getStyle("A4:$cellELast")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                            'color' => ['argb' => '000000'],
                        ],
                    ],
                    'alignment' => [
                        'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                        'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                    ],
                ]);

                // FOOTER
                $event->sheet->getDelegate()->getStyle("$stringTotal:$sumPendapatanBersih")->applyFromArray([
                    'font' => [
                        'bold' => true,
                        'size' => 12,
                    ],
                    'borders' => [
                        'allBorders' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                            'color' => ['argb' => '000000'],
                        ],
                    ],
                    'alignment' => [
                        'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                        'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                    ],
                ]);

            },

        ];
    }
}
